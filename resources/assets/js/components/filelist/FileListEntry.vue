<template>
    <tr>
        <td>
            <input type="checkbox" :checked="item.checked" v-model="checked" @change.prevent="toggleFile(item)">
        </td>
        <td>
            <span :class="'glyphicon glyphicon-' + this.icon"></span>
        </td>
        <td>
            <a @click.prevent="click">{{ item.basename }}</a>
        </td>
